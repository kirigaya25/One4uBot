...
async def last_fm(lastFM):
    ...
    if playing is not None:
        ...
        rectrack = sub("^", "https://open.spotify.com/search/",
                       rectrack)
        if image:
            output = f"[‎]({image})[{LASTFM_USERNAME}]({username}) __sedang mendengarkan:__\n\n• [{playing}]({rectrack})\n`{tags}`"
            preview = True
        else:
        ...
        playing = User(LASTFM_USERNAME, lastfm).get_now_playing()
        output = f"[{LASTFM_USERNAME}]({username}) __lagu yang Enako putar:__\n\n"
